...
Introduction d'un système de mouvement cohérent, discret et réutilisable dans toute l'app (vanilla JS, sans dépendance), puis branchement sur les interactions utilisateur les plus fréquentes.

- **Motion Foundation (3.2.1)** : nouveaux `css/motion-tokens.css` (durées, easings, distances, scales, stagger) et `css/motion-components.css` (`@keyframes motion-enter`, `.motion-enter(-subtle/-soft/-strong)`, `.motion-stagger`, `.interactive-surface`, filet `prefers-reduced-motion` global), linkés dans `index.html`/`login.html`. Consolidation : `tabFadeIn`, `dashboard-fade-in` (8 usages) et `modalFadeInScale` fusionnées dans `motion-enter` ; `@keyframes dashboard-float` (mort, jamais référencé) supprimé. Les 7 modales passent de `display:none` brutal à une transition `opacity`/`visibility` (ouverture ET fermeture animées, aucun JS touché — Echap/clic-backdrop intacts). Cascade Dashboard (header→hero→kpi→widget) tokenisée. Animations décoratives (`dashboard-hero-float`, `dashboard-pulse-soft`, `shimmer`, `spin`) conservées telles quelles (`index.html`, `login.html`, `styles.css`, nouveaux fichiers `css/motion-tokens.css` et `css/motion-components.css`).
- **User Feedback (3.2.2)** : `showMessage()` (toast, 64 appels dans l'app) gagne une entrée/sortie fluides sans changer sa signature ni ses durées (`.message--leaving` + `transitionend`/fallback). `saveCardEdits()` n'avait aucune protection contre le double-clic pendant la sauvegarde — bouton désormais désactivé + spinner `.loading` (déjà existant, même pattern que l'ajout de carte) pendant l'`await`, restauré sur erreur (`modules/utils.js`, `modules/card-detail.js`, `styles.css`).
- **Affinage des tokens** : sensation jugée trop "punchy" au démarrage après usage prolongé — easing d'entrée adouci (`cubic-bezier(0,0,0.2,1)` → `cubic-bezier(0.33,1,0.68,1)`, plus de rampe de démarrage), distance de translation par défaut réduite (`--motion-distance-md` 8px → 4px), durées légèrement rallongées dans la limite de +20% (`--motion-duration-normal` 220→260ms, `--motion-duration-emphasized` 400→480ms). Un seul fichier touché, changements appliqués un par un (`css/motion-tokens.css`).
- **Re-render Collection (3.2.3)** : ajout/suppression/tri/filtre/changement de vue grille↔tableau convergent tous vers `renderCollectionGrid()`/`renderCollectionTable()` (reconstruction `innerHTML` complète, sans transition). Nouveau helper local `replayEntrance()` qui réapplique `.motion-enter` (classe existante) sur le conteneur à chaque rebuild — un seul point de branchement couvre les 5 cas audités, plus de clignotement au changement de contenu. Limite assumée : c'est le bloc entier qui apparaît en fondu, pas la carte précise ajoutée/supprimée individuellement (nécessiterait de faire circuler l'id concerné dans le pipeline de rendu partagé — proposé pour un sprint futur, non implémenté) (`modules/collection.js`).

Aucune nouvelle dépendance, aucune logique métier modifiée, aucune régression fonctionnelle constatée (vérifié en navigateur à chaque étape : tab switching, modales, boutons, inputs, `prefers-reduced-motion`, mobile, console).
